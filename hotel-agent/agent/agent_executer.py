...
from a2a.types import (
    FilePart,
    FileWithBytes, FileWithUri, Part, TaskState, 
...
class HotelAgentExecuter(AgentExecutor):
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id="hotel_agent", new_message=new_message
        )

    async def _process_request(
            self, new_message: types.Content,
            session_id: str, task_updater: TaskUpdater
    ) -> None:
        ...
        async for event in self._run_agent(session_id, new_message):
            if event.is_final_response():
                ...
                logger.debug(f"Yielding final responseL {parts}")
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="hotel_agent", session_id=session_id
        )
